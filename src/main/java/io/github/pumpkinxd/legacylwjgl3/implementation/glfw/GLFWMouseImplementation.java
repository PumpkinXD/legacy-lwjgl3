...
public class GLFWMouseImplementation implements MouseImplementation {
    ...
    public void createMouse() {
        ...
        this.posCallback = GLFWCursorPosCallback.create((window, xpos, ypos) -> {
            double x =  xpos;
            double y = Display.getHeight() -  ypos; // I don't know why but this un-inverts the y motion of mouse inputs
            ...
            if (dx != 0 || dy != 0) {
                ...
                {//workaround , not a proper fix
                    if (last_x>Display.getWidth()&&x<Display.getWidth() || last_y>Display.getHeight()&&y<Display.getHeight()) {
                        dx=0;
                        dy=0;
                        accum_dx=0;
                        accum_dy=0;
                    }
                    if(last_x<0&&x>0||last_y<0&&y>0) {
                        dx=0;
                        dy=0;
                        accum_dx=0;
                        accum_dy=0;
